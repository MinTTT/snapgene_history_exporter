...
def tag_generator(idf_list: list, mode: str='date', prefix: Optional[str]=None ):
    """
    Generate a unique tag for each fragment
    the tag is generated by the date [by default, or specify via parameter `prefix`] and a number.

    Parameters
    ----------
    idf_list : list
        a list of identify strings for each fragment

    mode : str, optional
        the mode to generate the tag, by default 'date'
        - 'date': generate the tag by the date
        - 'prefix': generate the tag by the prefix
    
    prefix : Optional[str], optional
        the prefix to generate the tag, by default None. Only valid when mode is 'prefix'

    """
    if mode == 'date':
        tag_prefix = datetime.datetime.today().strftime('%m%d')
